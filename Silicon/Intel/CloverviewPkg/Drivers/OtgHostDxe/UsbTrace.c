  ...
  )
{
  ...
  (VOID)Sig;
  DEBUG ((DEBUG_ERROR, "OtgHostDxe: ACPI trace @%08x sig=%08x PS0=%u\n",
    CT_USB_TRACE_BASE, Sig, MmioRead32 (CT_USB_TRACE_BASE + 0x08)));
  if (Sig == CT_USB_TRACE_SIGNATURE) {
    ...
    DEBUG ((DEBUG_ERROR, "OtgHostDxe: no valid USB _PS0 trace; raw PS0 count is not valid\n"));
  }
  DEBUG_CODE_BEGIN ();
  MicroSecondDelay (5000000);
  DEBUG_CODE_END ();
  // Clear all 16 DWORDs, exactly like the old package. Only AML _PS0 writes
