#!/usr/bin/env bash
#
# Pack the t00gPkg (Asus ZenFone 6 T00G / A600CG, Intel Atom Z25xx) firmware
# into the stock OSIP boot image. Called by the repository build.sh after the
# edk2 build produced the FD; this is where device-specific boot image handling
# lives.
#
...
DEVICE="$(basename "$PKG_DIR")"     # t00gPkg
DEVICE="${DEVICE%Pkg}"              # t00g
