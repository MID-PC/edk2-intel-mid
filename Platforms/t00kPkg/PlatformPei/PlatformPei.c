...
#ifdef T00K_USB_TRACE
  // Same reserved-page mechanism as the old CloverTrailPkg USB trace.
  // Do not initialize this RAM here: OtgHostDxe must print the old contents.
  if ((CT_USB_TRACE_BASE < HighBase) ||
      ((UINT64)CT_USB_TRACE_BASE + CT_USB_TRACE_PAGE_SIZE > HighLimit) ||
      ((CT_USB_TRACE_BASE & EFI_PAGE_MASK) != 0) ||
      (CT_USB_TRACE_BASE != ConsoleBase + SIZE_4KB)) {
    DEBUG ((DEBUG_ERROR, "USB trace page is outside the supported RAM layout\n"));
    CpuDeadLoop ();
  }
  // Allocation is combined with the adjacent console page below.
#endif
...
#ifdef T00K_USB_TRACE
    (UINT64)SIZE_8KB, // 3EEFD000 console + 3EEFE000 CTTR, old package layout
#else
    (UINT64)SIZE_4KB,
#endif
    EfiReservedMemoryType
